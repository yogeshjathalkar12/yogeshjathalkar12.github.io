...
export default function CrmMarketing() {
  ...
  async function fetchData() {
    try {
      const [segRes, conRes, acctRes] = await Promise.all([
        supabase.from('audience_lists').select('*').order('created_at', { ascending: false }),
        supabase.from('contacts').select('*'),
        supabase.from('email_accounts').select('id, label'),
      ]);
      ...
      if (acctRes.data && acctRes.data.length > 0) setAccountId(acctRes.data[0].id);
    } catch (err) {
  ...
  async function handleDispatchEmail(e: React.FormEvent) {
    e.preventDefault();
    if (!accountId || !selectedSegment || !emailSubject.trim() || !emailBody.trim()) return;
    ...
    try {
      ...
      const resp = await fetch(`${EMAIL_API}/crm/dispatch`, {
        method: 'POST',
      ...
      if (!resp.ok) throw new Error(body.detail || `Could not dispatch (${resp.status}).`);

      if (!body.dispatched) {
        setDispatchError('No matching contacts had an email address on file — nothing was sent.');
        return;
      }

      const sentSoFar = body.first_batch?.sent ?? 0;
      setDispatchResult(
        `Sending to ${body.matched_contacts} contact${body.matched_contacts === 1 ? '' : 's'}` +
        (body.skipped_no_email ? ` (${body.skipped_no_email} skipped — no email on file)` : '') +
        `. ${sentSoFar} sent so far; the rest go out over the next few minutes at your warmup-paced rate.`
      );
      setEmailSubject('');
      setEmailBody('');
    } catch (err: any) {
      console.error('Failed to dispatch campaign:', err);
      setDispatchError(err.message || 'Could not dispatch this email.');
    } finally {
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.4rem' }}>
        ...
        <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '6px', padding: '1.2rem' }}>
          ...
          <form onSubmit={handleDispatchEmail}>
            {accounts.length === 0 ? (
              <div style={{ fontSize: '0.65rem', color: 'var(--red)', marginBottom: '0.8rem' }}>
                No sending account connected — connect one in the Email tool first, this dispatcher sends through it.
              </div>
            ) : (
              <>
            ...
            {dispatchError && (
              <div style={{ color: 'var(--red)', fontSize: '0.65rem', marginBottom: '0.8rem' }}>
                {dispatchError}
              </div>
            )}
            ...
            <button
              type="submit"
              disabled={dispatching || accounts.length === 0}
              style={{ width: '100%', background: 'var(--grad)', color: '#fff', border: 'none', padding: '0.65rem', borderRadius: '4px', cursor: accounts.length === 0 ? 'not-allowed' : 'pointer', opacity: accounts.length === 0 ? 0.5 : 1, fontFamily: 'var(--mono)', fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: '0.08em' }}
            >
