...
export default function WaConnectionSetup() {
  ...
  const [editingCatalogId, setEditingCatalogId] = useState<Record<string, string>>({});
  ...
  async function fetchAccounts() {
    try {
      const { data, error } = await supabase
        ...
        .select('id, label, phone_number_id, daily_cap, warmup_target, catalog_id, created_at')
  ...
  async function handleSaveCatalogId(accountId: string) {
    const value = (editingCatalogId[accountId] ?? '').trim();
    // catalog_id isn't a secret (it's just a Meta Commerce Catalog reference,
    // not a credential), so this is a plain RLS-protected Supabase update —
    // no need to route it through the backend like the access_token is.
    await supabase.from('whatsapp_accounts').update({ catalog_id: value || null }).eq('id', accountId);
    fetchAccounts();
  }
    ...
    <div>
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '6px', padding: '1.4rem', marginBottom: '1.6rem', maxWidth: 480 }}>
        ...
        <form onSubmit={handleConnect} style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
          ...
          <input style={fieldInputStyle} placeholder="Commerce Catalog ID (optional — enables product messages)" value={catalogId} onChange={(e) => setCatalogId(e.target.value)} />
      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.7rem', maxWidth: 620 }}>
          {accounts.map((a) => (
            <div key={a.id} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '6px', padding: '1rem 1.2rem' }}>
              ...
              <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                <input
                  style={{ ...fieldInputStyle, marginBottom: 0, flex: 1, fontSize: '0.6rem' }}
                  placeholder="Commerce Catalog ID (optional)"
                  value={editingCatalogId[a.id] ?? a.catalog_id ?? ''}
                  onChange={(e) => setEditingCatalogId((prev) => ({ ...prev, [a.id]: e.target.value }))}
                />
                <button
                  onClick={() => handleSaveCatalogId(a.id)}
                  style={{ background: 'transparent', border: '1px solid var(--border)', color: 'var(--dim)', padding: '0.4rem 0.8rem', borderRadius: '4px', cursor: 'pointer', fontSize: '0.6rem', flexShrink: 0 }}
                >
                  Save
                </button>
              </div>
